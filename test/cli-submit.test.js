...
test('CLI exposes submit auto loop without replacing serve mode', () => {
  ...
  assert.match(cli, /const \{ autoAdvanceRun, refreshRun, snapshotRun \} = await import\('\.\.\/src\/orchestrator\.js'\)/);
  ...
  assert.match(cli, /async function confirmFailureTerminal\(runId, state, readRunStatus, pollMs\)/);
  ...
  assert.match(cli, /--refresh\s+Refresh runner state before printing/);
